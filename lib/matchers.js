...
const TemporalValue = require('./TemporalValue')
...
function valueOf(val) {
	...
	if (val instanceof TemporalValue) {
		return new TemporalMatcher(val)
	}

	// a very weak way to determine if it's a later schedule object
	if (val && Array.isArray(val.schedules) && isFunction(val.customPeriod)) {
		return new TemporalMatcher(val)
	}
...
class TemporalMatcher extends Matcher {
	constructor(temporalValue) {
		super('temporal', later.schedule(temporalValue))
	}
...
module.exports.temporal = (val) => {
	return new TemporalMatcher(val)
}

module.exports.fromOperator = (operator) => {
	if (operator === '===') {
		return IsExactlyMatcher
	}
...
module.exports.TemporalMatcher = TemporalMatcher