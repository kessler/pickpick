 ...
 *
 *    ```js
 *    const { Experiment, Variation, Targeting, matchers } = require('pickpick')
 *    
 *    const e1 = Experiment.create({
 *    	name: 'my experiment',
 *    	id: 'foo',
 *    	variations: [
 *    		{ object: 1, weight: 1 }, // this is the same as Variation.create({ object: 1, weight: 1 })
 *    		{ object: 2, weight: 1 },
 *    		{ object: 3, weight: 1 }
 *    	],
 *    	targeting: '_.geo === "US"'
 ...
 */
class Experiment {
	constructor({ name, id, variations = [], targeting = Targeting.default(), userData }) {
		if (!id) {
		...
		if (!(targeting instanceof Targeting)) {
			if (!isString(targeting)) {
				throw new Error('targeting parameter can only be an instance of Targeting or a string (targeting expression)')
			}
	...
	static create({ name = 'FooExperiment', id, variations = [], targeting, userData }) {
