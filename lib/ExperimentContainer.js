...
let _isIdExists = Symbol('_isIdExists')
...
class ExperimentContainer {
	...
	add(...experiments) {
		for (let experiment of experiments) {
			...
			let experimentId = experiment.id || experiment.object.id
			if (this[_isIdExists](experimentId)) {
				throw new Error(`experiment id (${experiment.id}) must be unique`)
			}
	...
	has(experiment) {
		if (!(experiment instanceof Experiment)) throw new Error('has accepts only Experiment instances')
		for (let containerExperiment of this) {
			if (experiment === containerExperiment) return true
		}
		return false
	}

	[_isIdExists](experimentId) {
		for (let containerExperiment of this) {
