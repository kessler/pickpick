...
class Targeting {
	...
	_parseMatchers() {
		...
		for (let [key, matcher] of this[_expression]) {
			if (!(matcher instanceof matchers.Matcher)) {
				if (validateMatcherObject(matcher)) {
					let requestedMatcher = matchers[matcher.matcher]
					this[_expression].set(key, requestedMatcher(matcher.value))
				} else {
					throw new Error(`matcher object not formatted as expected -> ${matcher}`)
				}
			}
		}
	}
...
// validate all the required properties of the matcher object exist
function validateMatcherObject(matcherObj) {
	if (typeof matcherObj.matcher === 'undefined'
		|| typeof matcherObj.value === 'undefined') {
		console.log(`mathcer missing required properties -> ${matcherObj}`)
		return false
	}

	if (matchers[matcherObj.matcher] === 'undefined') {
		console.log(`matcher not found for object -> ${mathcerObj}`)
		return false
	}
